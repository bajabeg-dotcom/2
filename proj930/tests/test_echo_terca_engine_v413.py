...
def test_ordinary_second_solo_is_not_auto_promoted_to_terca():
    main_t=track(0,'Main Solo'); aux_t=track(1,'Second Solo')
    main=[note(60,0,180,100),note(62,480,660,100),note(64,960,1140,100),note(65,1440,1620,100)]
    # Strong parallel thirds, but this is still named/owned as another solo, not a terca layer.
    aux=[note(64,0,180,85),note(65,480,660,85),note(67,960,1140,85),note(69,1440,1620,85)]
    groups=[{'role':'solo','confidence':.95,'track':main_t,'channel':0,'notes':main,'trackName':'Main Solo'},
            {'role':'solo','confidence':.88,'track':aux_t,'channel':0,'notes':aux,'trackName':'Second Solo'}]
    stats=Counter()
    r=ste.optimize_existing_echo_terca(groups,{},1920,480,{'solo-prof':PROFILE},{},stats)
    assert not any(x['kind']=='third' for x in r)
